<?php $exChoii = explode('|', $tsk['choices']);
	...
	if ($_GET['act'] == 'submit') {
		...
		$myResult = _content($_POST['task-result-'.$tsk['id']]);
		if ($myResult == $tsk['result']) $corect = 'yes';
		if ($checkSubmit <= 0 && $mySolution) {
			$ad = mysql_query("INSERT INTO `daily_ex_submit` (`answer`, `result`, `uid`, `iid`, `correct`, `time`) VALUES ('$mySolution', '$myResult', '$u', '$e', '$corect', '$current')");
...
<? } ?>
					...
					<div class="hide tab-indexs solve-problem">
<? 	 if ($checkSubmit <= 0) {
		if ($tsk['type'] == 'test') {
			echo '<dl class="line"><dt>Result *</dt> <dd class="task-radio-option">';
			for ($j = 0; $j < count($exChoii); $j++) {
				echo '<label class="radio"><input type="radio" id="option'.$j.$tsk['id'].'" value="'.$exChoii[$j].'" name="task-result-'.$tsk['id'].'"/> '.$exChoii[$j].'</label>';
			}
			echo '</dd></dl>';
		} else if ($tsk['type'] == 'answer') {
			echo '<dl class="line line-mar"><dt>Result *</dt> <dd><input type="text" name="task-result-'.$tsk['id'].'" class="required" placeholder="Task result" value="'.$tskEx['result'].'"/></dd></dl>';
		}
		echo '<div class="daily-task-solution"><h4 style="margin-bottom:8px">Input your solution *</h4> <textarea name="task-solution-'.$tsk['id'].'" class="required" style="height:160px">'.$tskEx['solution'].'</textarea></div>';
	} else {
		if ($tsk['type'] == 'test') {
			echo '<dl class="line"><dt>Result *</dt> <dd class="task-radio-option">';
			for ($j = 0; $j < count($exChoii); $j++) {
