...
$rCode = getRecord('course_test', "`id` = '$e'");
...
<form class="borderwrap score-form" data-action="<?php echo 'c='.$c.'&t=score&e='.$e.'&u='.$uid ?>" method="post">
	<div class="name">
	...
	if (!$mResult) $mResult = '[empty]'; ?>
		<div class="one-ans-score-sheet rows <? if ($mResult == '[empty]') echo 'empty'; else if ($mResult == $rResult) echo 'correct'; else echo 'wrong' ?>">
			...
			<? if ($rAns == 'ans') { ?>
				...
				<div class="exam-answer my-solution"><? echo $mAnswer ?></div>
			<? } ?>
		</div>
<? } ?>
	</div>
