<?php $qList = $getRecord -> GET($typ, "`did` IN ($childDotStr)", '%30', '');
...
<div class="row row-lib <?php if ($qList['solve'] == 'solve') echo 'solved'; if ($u == $qList['uid'] /*|| countRecord($startb, "`uid` = '$u' AND `iid` = '{$qList['id']}'") > 0*/ ) echo ' stared'; if ($u != $qList['uid'] && countRecord($typ.'_cmt', "`uid` = '$u' AND `iid` = '{$qList['id']}'") > 0 ) echo ' joined' ?>">
	...
	<div class="lib-list-content">
		<a href="<?php echo $link.$qList['id'] ?>">
			...
			<span class="right label-available label label-<?php if ($avai == 'student') echo 'info'; else if ($avai == 'teacher') echo 'danger'; else echo 'primary' ?> capitalize"><?php echo $avai ?></span>
	...
	<div class="lib-sta right">
		...
		<div class="lib-sta-rep"><i class="fa fa-comments"></i> <?php echo countRecord($typ.'_cmt', "`iid` = '{$qList['id']}' ") ?></div>
