...
<div class="lesson-content" style="margin-left:270px">
	...
	<?php if ($recordLesson == 'Failed server') echo '<div class="alerts alert-error">Failed to get any response. Maybe we can\'t contact the BBB server</div>';
		...
		echo '<div class="close-interact-lesson"></div><iframe class="interact-lesson-iframe play-record" src="'.$recordLesson.'"></iframe>';
		if ($u == $cInfo['uid']) changeValue('lesson', "`id` = '$l'", "`run` = 'yes'");
	} ?>
		</div>
		<div class="name">
			Lesson content
