...
//	$exTags = $_POST['ex-tags'];
	...
	$exInput = $exOutput = $exCode = $exInputFile = $exOutputFile = $exTestCaseDir = '';
	if ($exType == 'program') {
...
//							echo $bigfolder;
	...
	if (countRecord('ex', "`type` = '$exType' AND (`title` = '$exTitle' OR `quest` = '$exQuest')") > 0) {
		$qInfo = getRecord('ex', "`type` = '$exType' AND (`title` = '$exTitle' OR `quest` = '$exQuest')");
		if (countRecord('ex', "`type` = '$exType' AND `title` = '$exTitle'") > 0) echo '<div class="alerts alert-error">There\'s already a same-type exercise with this title. Please check here <a href="#!exercise?q='.$qInfo['id'].'">'.$qInfo['title'].'</a></div>';
		...
		if ($exType == 'program') {
			$exFinalSolutionToAdd = $exSolutionDir;
			if (file_exists($testPath)) $exTestCaseDir = $tDir.'testcase/';
			else $exTestCaseDir = '';
		} else $exFinalSolutionToAdd = $exSolution;
		$a = mysql_query("INSERT INTO `ex` (`uid`, `code`, `type`, `did`, `title`, `quest`, `answer`, `choices`, `result`, `tid`, `available`, `language`, `language_code`, `ace_code`, `input`, `output`, `testcase`, `time`) VALUES ('$u', '$exCode', '$exType', '$dot', '$exTitle', '$exQuest', '$exFinalSolutionToAdd', '$exChoice', '$exResult', '$exTopics', '$exAvailable', '$exLangName', '$exLC', '$exLang', '$exInputFile', '$exOutputFile', '$exTestCaseDir', '$current')");
//		echo '<br/>'.$testZipPath.'<br/>'.$exCode.'~~~'.$exLC.'<br/>'.$exSolution;
		if ($a) {
			...
			$newEx = getRecord('ex', "`code` = '$exCode' AND `uid` = '$u' ");
			addRep($u, 5);
			activityAdd('create-ex', $newEx['id']);
