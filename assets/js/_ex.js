	...
	$('.e-my-solution').click(function () {
		...
		$('.more-solution-detail#s'+id).load(url + ' .more-solution-detail#s'+id+' > div', function () {
			...
			$('.close-edit-my-solution').click(function () {
				$('.more-solution-detail').load(paths + " .more-solution-detail > div");
				$('.tooltip').remove();
			});
...
//		$('.test-solution').load(path + " .test-solution > form", function () {
//		sce(); flatApp();
		...
		$('form.add-solution').submit(function () {
			if (type == 'program') {
				var ace_editor = ace.edit('ace-editor');
				$('.textarea-add-my-solution').val('<code>' + ace_editor.getSession().getValue() + '</code>');
			}
			$.ajax({
				url: path + '&go=submit',
				type: 'POST',
