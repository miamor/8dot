function submit_form_ex () {
	$('form.form-new-lib#exercise').submit(function () {
		$form = $('form.form-new-lib#exercise');
...
//			alert($thisStep.attr('class'));
		...
		if (checkReturn == true) {
			$.ajax({
				url: MAIN_URL + '/pages/exercise.php?mode=new&act=submit',
				type: 'POST',
				data: formData,
...
$(function () {
	$('.form-new-lib#exercise').each(function () {
		step_settings(this);
		$(this).find('.step-1 .next-step').hide()
	});

	// Step 1
	$('.form-new-lib#exercise .step-1 .choose-type').not('.disabled').each(function () {
...
//	$('.step-2 .next-step').show();
	$('.form-new-lib#exercise .step-1 .next-step').click(function () {
		type = $('input.ex-type').val();
		nums = $('.ex-num_'+type).val();
//		$('.form-new-lib#exercise').find('.step-2').prepend(nums + ' ex');
		$('.form-new-lib #'+type).removeClass('hide').show();
		$('.form-new-lib #'+type).find('input, select').not(':checkbox,:radio').parent('dd').parent('dl').addClass('line-mar');
