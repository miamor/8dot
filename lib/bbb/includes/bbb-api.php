...
function bbb_load_file($url) {
	...
	if (extension_loaded('curl')) {		
		...
		$data = curl_exec( $ch );
		$isError = (curl_getinfo($ch, CURLINFO_HTTP_CODE) == 404);
		curl_close( $ch );
...
class BigBlueButton {
	...
	*/
		...
		$this->_bbbServerUrl		= CONFIG_SERVER_URL;
	}
	...
	*/
	...
	public function getJoinMeetingURL($joinParams) {
		...
		*/
		...
		if (((isset($joinParams['configToken'])) && ($joinParams['configToken'] != ''))) {
			$params .= '&configToken='.urlencode($joinParams['configToken']);
		}
		// Return the URL:
	...
	public function getConfigXml ($configName) {
		$BASE_URL = $this -> _bbbServerUrl;
		$result =  bbb_load_file( $BASE_URL.'client/conf/'.$configName.'.xml');
		
		if (is_null($result)) return 'Error : file not found on server.';
		else return (new SimpleXMLElement($result));
	}

	
	public function setConfigXmlForMeeting ($meetingID, $xmlConfig) {
		$SALT = $this->_securitySalt;
		$URL = $this->_bbbServerBaseUrl;
		$url_join = $URL."api/setConfigXML.xml?";
		
		//$params = 'meetingID='.urlencode($meetingID).'&checksum='.sha1($meetingID.BigBlueButton::encodeURI($xmlConfig).$SALT).'&configXML='.urlencode(BigBlueButton::encodeURI($xmlConfig));
		$baseString = 'configXML='.urlencode(BigBlueButton::encodeURI($xmlConfig)).'&meetingID='.urlencode($meetingID);
	...
	public function getJoinMeetingURLConfigXml ($joinParams) {
		$this->_meetingId = $this->_requiredParam($joinParams['meetingId'], 'meetingId');
		$this->_username = $this->_requiredParam($joinParams['username'], 'username');
		$this->_password = $this->_requiredParam($joinParams['password'], 'password');	
		// Establish the basic join URL:
		$joinUrl = $this->_bbbServerBaseUrl."api/setConfigXML?";
		// Add parameters to the URL:
		$params = 
		'meetingID='.urlencode($this->_meetingId).
		'&fullName='.urlencode($this->_username).
		...
		// Return the URL:
		return ($joinUrl.$params.'&checksum='.sha1("setConfigXML".$params.$this->_securitySalt));
	}
	
	public function getJoinURLConfigXml ($joinParams, $configToken) {
		$SALT = $this-> _securitySalt;
		$URL = $this-> _bbbServerBaseUrl;
		$url_join = $URL."api/join?";
		$meetingID = $joinParams['meetingId'];
		$userName = $joinParams['username'];
		$PW = $joinParams['password'];
		$params = 'meetingID='.urlencode($meetingID).'&fullName='.urlencode($userName).'&password='.urlencode($PW).'&configToken='.urlencode($configToken);
