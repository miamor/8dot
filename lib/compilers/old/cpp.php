...
*/
	...
	if (trim($error) == "" || !strpos($error, "error")) {
		if (strpos($error, "error")) echo "<div class='console error console-bottom bold'>Errors fetched:</div> <div class='console error non-icon'>$error</div>";

		if (trim($input) == "") $execOut = $out;
		else $execOut = $out." < ".$filename_in;
		...
		if (file_exists($testcasePath)) {
			...
			if ($handle = opendir($testcaseInputPath)) {
				while (false !== ($file = readdir($handle))) {
					if (!in_array($file, $blacklist)) {
						$moreout = $out." < ".$testcaseInputPath.$file;
	...
	if ($output) {
		...
		if (file_exists($testcasePath)) {
			...
			else if ($okPercent >= 30 && $okPercent < 100) echo "<div class='console warning console-bottom bold'>Percentage of correct tests: $okPercent%</div>";
		}
